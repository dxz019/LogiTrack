...
class Order {
  static async create({
    ...
    // First, we'll use Google Routes API to get distance and duration? 
    // But for now, we'll leave them null and update later via service.
    // However, the spec says to store estimated_distance and estimated_duration from Routes API.
    // We'll create the order with null for those and update later.
    const result = await pool.query(
      `INSERT INTO orders (
        customer_id, pickup_address, pickup_location, delivery_address, delivery_location,
        package_description, priority, notes
      ) VALUES (
        $1, $2, ST_SetSRID(ST_MakePoint($4, $3), 4326),
  ...
  static async updateDistanceDuration(orderId, distance, duration) {
    const result = await pool.query(
      `UPDATE orders SET estimated_distance = $1, estimated_duration = $2, updated_at = NOW() WHERE id = $3 RETURNING *`,
      [distance, duration, orderId]
    );
    return result.rows[0];
  }
